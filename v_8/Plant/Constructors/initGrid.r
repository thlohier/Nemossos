initGrid <- function(inCoord, inR_shoot, inSoil)
{
	...
	x_inf_tmp <- inCoord[1] - inR_shoot/inSoil$cel_size;
	x_sup_tmp <- inCoord[1] + inR_shoot/inSoil$cel_size;
	...
	if(x_inf_tmp > 0 && x_sup_tmp < inSoil$width/inSoil$cel_size)
	{
		...
		b <- -2*inCoord[2];
		C <- inCoord[1]^2 + inCoord[2]^2 - (round(inR_shoot/inSoil$cel_size,0))^2 + X^2 -2*inCoord[1]*X;
	...
	else if(x_inf_tmp < 0 && x_sup_tmp < inSoil$width/inSoil$cel_size)
	{
		...
		b <- -2*inCoord[2];
		C <- inCoord[1]^2 + inCoord[2]^2 - (round(inR_shoot/inSoil$cel_size,0))^2 + x1^2 -2*inCoord[1]*x1;
		
		y1 <- cbind(round(((-b - sqrt(b^2 - 4*C))/2),0),round(((-b + sqrt(b^2 - 4*C))/2),0));
		
		
		## Cylinder portion outside the patch (tor)
		x2 <- round(inSoil$width/inSoil$cel_size - (inR_shoot/inSoil$cel_size - inCoord[1]),0):(inSoil$width/inSoil$cel_size);
		
		b <- -2*virt_cent[2];
		C <- virt_cent[1]^2 + virt_cent[2]^2 - (round(inR_shoot/inSoil$cel_size,0))^2 + x2^2 -2*virt_cent[1]*x2;
	...
	else if(x_inf_tmp > 0 && x_sup_tmp > inSoil$width)
	{
		...
		## Cylinder portion outside the patch (tor)
		x2 <- 0:round(inR_shoot/inSoil$cel_size - (inSoil$width/inSoil$cel_size - inCoord[1]),0);
