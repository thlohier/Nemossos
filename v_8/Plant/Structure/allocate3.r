allocate <- function(inN_days, inD, inH, inPeriod, inOutSoil, inOutPool, inOutDebug)
{
	...
	for(indiv in 1:length(inOutPool))
	{
		...
		if(NPP > 0)
		{
			...
			# print("-------------- Allocation --------------")
			# print(sum(inOutPool[[indiv]]$traits$N_up[inD,],na.rm=TRUE))
			print(NPP*inOutPool[[indiv]]$physio$a_N*inOutPool[[indiv]]$physio$LNC_max)
			
			if(inOutPool[[indiv]]$traits$N_need[inD] > 0)
			{
				a <- max(0,((sum(inOutPool[[indiv]]$traits$N_up[inD,],na.rm=TRUE)/inOutPool[[indiv]]$traits$N_need[inD]) - inOutPool[[indiv]]$physio$a_N)/(1 - inOutPool[[indiv]]$physio$a_N));
			}
			else
				a <- 1;
			# a <- max(0,(sum(inOutPool[[indiv]]$traits$N_up[inD,],na.rm=TRUE) - NPP*inOutPool[[indiv]]$physio$a_N*inOutPool[[indiv]]$physio$LNC_max)/(NPP*inOutPool[[indiv]]$physio$LNC_max*(1 - inOutPool[[indiv]]$physio$a_N)));
		...
		if(d_cur <= (inN_days+1))
		{
			...
			inOutPool[[indiv]]$B_shoot[d_cur] <- inOutPool[[indiv]]$B_shoot[d_cur] + a*NPP;
			
			inOutPool[[indiv]]$B_root[d_cur]  <- inOutPool[[indiv]]$B_root[d_cur] + (1-a)*NPP - 3600*inPeriod*30*1e-6*R_d_r*sum(inOutPool[[indiv]]$B_root,na.rm=TRUE);
